...
2. **`core/orm`** holds model metadata, SQL generation, CRUD, and access checks. Addon code should use **`sumeru/core/base`** instead of importing ORM internals directly.
...
5. Built-in kernel behavior for users/companies lives in the **`base`** addon under **`sumeru/addons/base/`** (Go models + XML). The tiny **`core/base/base`** tree is the filesystem **`base`** manifest only.
...
| `core/base` | **Stable API** for addons (`RegisterModel`, `Search`, …) |
