...
func TestEncodeActionResultClose(t *testing.T) {
	got, ok := EncodeActionResult("close").(map[string]interface{})
...
func TestEncodeActionResultOpenDialog(t *testing.T) {
	got, ok := EncodeActionResult("/web?model=crm.lead.lost&id=12&view_type=form").(map[string]interface{})
	if !ok {
		t.Fatalf("type: %#v", got)
	}
...
func TestEncodeActionResultRedirect(t *testing.T) {
	got, ok := EncodeActionResult("/web?action=5&menu_id=2").(map[string]interface{})
...
func TestActionOpenURL(t *testing.T) {
	u := ActionOpenURL("crm.lead.lost", 9)
